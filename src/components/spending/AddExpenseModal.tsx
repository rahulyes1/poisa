...
export default function AddExpenseModal({ isOpen, onClose }: AddExpenseModalProps) {
  ...
  const onSubmit = (event?: FormEvent, forceAdd = false) => {
    ...
    if (!forceAdd) {
      const isDuplicate = expenses.some(
        (e) => e.amount === parsedAmount && e.category === categoryLabel && e.date === date,
      );
      if (isDuplicate) {
    ...
    <div className="poisa-form-overlay flex items-end sm:items-center justify-center">
      <div className="poisa-form-panel w-full sm:max-w-[440px] h-[100dvh] sm:h-auto sm:max-h-[92dvh] overflow-y-auto pb-[calc(env(safe-area-inset-bottom)+96px)]">
        <div className="sticky top-0 z-10 bg-[#0d1117] border-b border-[#2d333b] px-4 py-3 flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={() => onSubmit(undefined, showDuplicateWarning)}
        ...
        <div className="fixed inset-x-0 bottom-[calc(env(safe-area-inset-bottom)+68px)] sm:bottom-6 sm:left-1/2 sm:w-[420px] sm:-translate-x-1/2 px-4 z-[65] space-y-2">
          {showDuplicateWarning && (
            <div className="rounded-xl border border-[#ffb347]/40 bg-[#ffb347]/10 px-3 py-2 flex items-center justify-between gap-2">
              <p className="text-xs text-[#ffb347]">Looks like a duplicate entry. Add anyway?</p>
              <button
                type="button"
                onClick={() => onSubmit(undefined, true)}
                className="shrink-0 text-xs font-semibold text-[#ffb347] underline"
              >
                Yes, add
              </button>
            </div>
